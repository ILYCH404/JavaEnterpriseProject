...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        for (UserMeal meal : meals) {
            if (startTime.isBefore(LocalTime.from(meal.getDateTime())) && endTime.isAfter(LocalTime.from(meal.getDateTime()))) {
                list.add(new UserMealWithExcess(meal.getDateTime(), meal.getDescription(), meal.getCalories(), meal.getCalories() < caloriesPerDay));
            }
        }
        return list;
    }
